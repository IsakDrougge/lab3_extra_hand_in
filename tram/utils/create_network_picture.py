...
MY_TRAMNETWORK_JSON = '/Users/isakdrougge/Desktop/Skola/Advanced-Python/labs-advPyth/data/tramnetwork.json'  # JSON file from lab1
TRAM_URL_FILE = '/Users/isakdrougge/Desktop/Skola/Advanced-Python/labs-advPyth/Lab3/tram/utils/tramstop_google_url.json'  # given in lab3/files, replaced with my own "VT" for västtrafik
...
def scaled_position(network):
    minlat, minlon, maxlat, maxlon = float(network.extreme_positions()['min_lat']), float(network.extreme_positions()['min_lon']), float(network.extreme_positions()['max_lat']), float(network.extreme_positions()['max_lon'])
...
def network_graphviz(network, outfile=MY_GBG_SVG, positions=scaled_position):
    ...
    for stop in network.all_stops():
        ...
        # x, y = float(stop.get_position()['lat']), float(stop.get_position()['lon'])
        x, y = float(stop.get_position()['lon']), float(stop.get_position()['lat'])
        print(x,y)
        if positions:
        ...
        dot.node(stop.get_name(), label=stop.get_name(), shape='rectangle', pos=pos_x + ',' + pos_y +'!',
            ...
            URL=stop_url(stop.get_name()),
            fillcolor=col, style='filled')
        
    for line in network.all_lines():
        stops = network.line_stops(line.get_number())
        for i in range(len(stops)-1):
            dot.edge(stops[i], stops[i+1],
                         color=gbg_linecolors[int(line.get_number())], penwidth=str(2))
